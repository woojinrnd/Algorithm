    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            ...
            if (MAP[i][j] == 1) {
                ...
                Visit[i][j] = -1; // 치즈가 있는 위치를 방문하지 않음으로 표시
            }
        }
    }
}
...
void Find_Cheese_List() {
    ...
    for (int i = 0; i < V.size(); i++) {
        if (V[i].second == true) continue; // 이미 녹은 치즈에 대해서는 탐색할 필요 없음.
        ...
        for (int j = 0; j < 4; j++) {
            ...
            // 단순히, MAP[nx][ny] == 0이 아닌, Visit의 값으로 외부공기인지 내부공기인지 체크도 해줘야 한다.
        }
...
bool Clear() {
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            if (MAP[i][j] == 1) return false; // 치즈가 남아 있는 경우
        }
    }
    return true; // 치즈가 모두 녹은 경우
}
